...
class RealmDecimal128(ctypes.Structure):
    _fields_ = [("w", ctypes.c_uint64 * 2)]  # uint64_t w[2]
...
class RealmObjectId(ctypes.Structure):
    _fields_ = [("bytes", ctypes.c_uint8 * 12)]  # uint8_t bytes[12]


# Structure definition for realm_uuid_t
class RealmUuid(ctypes.Structure):
    _fields_ = [("bytes", ctypes.c_uint8 * 16)]  # uint8_t bytes[16]
...
def get_values_from_kwargs(kwargs, num_properties):
    values_arr = (RealmValue * num_properties)()
    i = 0
    for arg in kwargs:
        """
        convert the value
        """
        values_arr[i] = convert_pyobject_value_to_realm_object_value(kwargs[arg])
        i += 1
