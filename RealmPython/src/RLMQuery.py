...
from RLMObjectModel import Property_Info
import re
...
def __get_all_realm_results__(realm_handle, class_key):
    ...
    res = rlm_lib.realm_object_find_all(
        ctypes.c_void_p(realm_handle), ctypes.c_uint32(class_key)
    )
    if res != None:
        return res
    else:
...
def __parse_query_for_results__(realm_results_t, query_string):
    ...
    res = rlm_lib.realm_query_parse_for_results(
        ctypes.c_void_p(realm_results_t),
        ctypes.c_char_p(query_string.encode("utf-8")),
        ctypes.c_size_t(num_args),
        ctypes.byref(arg_t_arr),
    )
    if res != None:
        return res
    else:
...
def __realm_query_find_first__(realm_query_t):
    ...
    res = rlm_lib.realm_query_find_first(
        ctypes.c_void_p(realm_query_t), ctypes.byref(out_value), ctypes.byref(out_found)
    )
    if res == True:
        return out_value
    else:
...
def __count_num_results__(results_handle):
    ...
    res = rlm_lib.realm_results_count(
        ctypes.c_void_p(results_handle), ctypes.byref(out_count)
    )
    if res == True:
        return out_count
    else:
...
def __get_objects_from_results__(results_handle, index_of_object):
    ...
    return rlm_lib.realm_results_get_object(
        ctypes.c_void_p(results_handle), ctypes.c_size_t(index_of_object)
    )
...
def __realm_get_new_results_handle__(old_results_handle, query_t_handle):
    ...
    res = rlm_lib.realm_results_filter(
        ctypes.c_void_p(old_results_handle), ctypes.c_void_p(query_t_handle)
    )
    if res != None:
        return res
    else:
...
def __realm_find_property__(realm_handle, class_key, property_name):
    temp_prop_info = Property_Info()
    out_found = ctypes.c_bool()
    rlm_lib.realm_find_property.restype = ctypes.c_bool
    res = rlm_lib.realm_find_property(
        ctypes.c_void_p(realm_handle),
        ctypes.c_uint32(class_key),
        property_name.encode("utf-8"),
        ctypes.byref(out_found),
        ctypes.byref(temp_prop_info),
    )
    if res == True:
        return temp_prop_info.key
    else:
        check_error()
