...
class ObjectModel:
    ...
    def __get_subclass_attributes__():
        for subcls in ObjectModel.__get_subclasses__():
            yield (
                subcls.__name__,
                inspect.getargspec(subcls.__init__).args[1:],
                inspect.getargspec(subcls.__init__).defaults,
            )
    ...
    def __get_rlm_schema_objects__():
        ...
        for (
            ...
            cls_args,
            cls_defaults,
        ) in ObjectModel.__get_subclass_attributes__():
            ...
            if cls_defaults != None:
                ...
                raise AttributeError(
                    "Realm Error: Please ensure all objects are defined properly"
                )
            x += 1
        ...
        for i in range(len(__schema_properties__)):
            all_props_arr[i] = ctypes.cast(
                __schema_properties__[i], ctypes.POINTER(cast_array)
            )
